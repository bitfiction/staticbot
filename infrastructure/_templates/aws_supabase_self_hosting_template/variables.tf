...
variable "supabase_public_url" {
  description = "Public URL for Supabase"
  type        = string
}
...
variable "anon_key_asymmetric" {
  description = "Pre-signed ES256 JWT for anon role. Used internally by Kong when opaque keys are configured."
  type        = string
  sensitive   = true
  default     = ""
}
...
variable "service_role_key_asymmetric" {
  description = "Pre-signed ES256 JWT for service_role. Used internally by Kong when opaque keys are configured."
  type        = string
  sensitive   = true
  default     = ""
}
...
variable "storage_tenant_id" {
  description = "Storage tenant ID (equivalent to project_ref for S3 protocol)"
  type        = string
  default     = "stub"
}
